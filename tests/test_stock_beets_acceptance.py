"""Stock Beets Integration Acceptance Test Suite.

Verifies end-to-end compatibility between stock Beets web server (beetsplug.web)
and the WebManager integration plugin (beetsplug.webmanager) using the real stock
Docker container: lscr.io/linuxserver/beets:latest
"""

import os
import shutil
import tempfile
import time
import json
import uuid
import subprocess
import urllib.request
import urllib.parse
import urllib.error
import pytest

from beets.library import Library, Item, Album
from beetsplug.web import app as beets_web_app
from beetsplug.webmanager import WebManagerPlugin
from beetsplug.webmanager.auth import set_api_key_file
import beetsplug.webmanager.operations as ops_mod

STOCK_BEETS_IMAGE = "lscr.io/linuxserver/beets:latest"


def _create_synthetic_audio(file_path: str, title: str, artist: str, album: str):
    """Generate a minimal valid audio file with tags for importer testing."""
    import wave
    import struct
    import mutagen.wave
    import mutagen.id3

    with wave.open(file_path, "w") as w:
        w.setnchannels(1)
        w.setsampwidth(2)
        w.setframerate(44100)
        w.writeframes(struct.pack("<h", 0) * 44100)

    try:
        mw = mutagen.wave.WAVE(file_path)
        mw.add_tags()
        mw.tags.add(mutagen.id3.TIT2(encoding=3, text=[title]))
        mw.tags.add(mutagen.id3.TPE1(encoding=3, text=[artist]))
        mw.tags.add(mutagen.id3.TALB(encoding=3, text=[album]))
        mw.save()
    except Exception:
        pass


@pytest.fixture
def in_process_acceptance_env():
    with tempfile.TemporaryDirectory() as td:
        music_dir = os.path.join(td, "music")
        downloads_dir = os.path.join(td, "downloads")
        config_dir = os.path.join(td, "config")
        os.makedirs(music_dir, exist_ok=True)
        os.makedirs(downloads_dir, exist_ok=True)
        os.makedirs(config_dir, exist_ok=True)

        dbpath = os.path.join(config_dir, "musiclibrary.blb")
        lib = Library(dbpath, directory=music_dir)

        # Set up API key
        key_file = os.path.join(config_dir, ".webmanager_api_key")
        token = "stock_beets_test_key_xyz987"
        with open(key_file, "w", encoding="utf-8") as f:
            f.write(token + "\n")
        set_api_key_file(key_file)
        ops_mod.set_allowed_roots([music_dir, downloads_dir, td])

        # Configure Beets web app
        plugin = WebManagerPlugin()
        beets_web_app.config["lib"] = lib
        beets_web_app.config["INCLUDE_PATHS"] = True
        beets_web_app.config["READONLY"] = True
        beets_web_app.config["TESTING"] = True

        # Pre-populate sample library data
        album = Album(
            album="Echoes of Time",
            albumartist="Chronos",
            year=2023,
            genre="Ambient",
        )
        lib.add(album)

        sample_file = os.path.join(music_dir, "track1.mp3")
        with open(sample_file, "wb") as f:
            f.write(b"ID3\x03\x00\x00\x00\x00\x00\x00MP3_DATA_PAYLOAD")

        item1 = Item(
            title="Sands of Time",
            artist="Chronos",
            album="Echoes of Time",
            albumartist="Chronos",
            album_id=album.id,
            track=1,
            genre="Ambient",
            year=2023,
            path=sample_file.encode("utf-8"),
        )
        lib.add(item1)
        lib._connection().commit()

        with beets_web_app.test_client() as client:
            yield {
                "client": client,
                "token": token,
                "lib": lib,
                "dirs": {
                    "root": td,
                    "music": music_dir,
                    "downloads": downloads_dir,
                    "config": config_dir,
                },
                "album": album,
                "item": item1,
                "sample_file": sample_file,
            }

        try:
            lib._connection().close()
        except Exception:
            pass
        ops_mod.set_allowed_roots(None)


def test_native_upstream_reads(in_process_acceptance_env):
    """Verify native upstream endpoints match the expected Beets REST API contracts."""
    client = in_process_acceptance_env["client"]
    item = in_process_acceptance_env["item"]
    album = in_process_acceptance_env["album"]

    # 1. GET /stats
    res = client.get("/stats")
    assert res.status_code == 200
    stats = res.get_json()
    assert stats["items"] == 1
    assert stats["albums"] == 1

    # 2. GET /artist/
    res = client.get("/artist/")
    assert res.status_code == 200
    artists = res.get_json()
    assert "artist_names" in artists
    assert "Chronos" in artists["artist_names"]

    # 3. GET /item/
    res = client.get("/item/")
    assert res.status_code == 200
    items_data = res.get_json()
    assert "items" in items_data
    assert len(items_data["items"]) == 1
    assert items_data["items"][0]["title"] == "Sands of Time"

    # 4. GET /item/<id>
    res = client.get(f"/item/{item.id}")
    assert res.status_code == 200
    item_res = res.get_json()
    assert item_res["title"] == "Sands of Time"

    # 5. GET /album/
    res = client.get("/album/")
    assert res.status_code == 200
    albums_data = res.get_json()
    assert "albums" in albums_data
    assert len(albums_data["albums"]) == 1
    assert albums_data["albums"][0]["album"] == "Echoes of Time"

    # 6. GET /album/<id>?expand
    res = client.get(f"/album/{album.id}?expand")
    assert res.status_code == 200
    expanded = res.get_json()
    assert expanded["album"] == "Echoes of Time"
    assert "items" in expanded
    assert len(expanded["items"]) == 1
    assert expanded["items"][0]["title"] == "Sands of Time"

    # 7. GET /item/values/<key>
    res = client.get("/item/values/artist")
    assert res.status_code == 200
    values = res.get_json()
    assert "values" in values
    assert "Chronos" in values["values"]


def test_native_file_stream(in_process_acceptance_env):
    """Verify native audio binary streaming endpoint."""
    client = in_process_acceptance_env["client"]
    item = in_process_acceptance_env["item"]

    res = client.get(f"/item/{item.id}/file")
    assert res.status_code == 200
    assert b"MP3_DATA_PAYLOAD" in res.data


def test_webmanager_plugin_security_and_operations(in_process_acceptance_env):
    """Verify WebManager plugin authentication, status, modification, and path containment."""
    client = in_process_acceptance_env["client"]
    token = in_process_acceptance_env["token"]
    item = in_process_acceptance_env["item"]
    dirs = in_process_acceptance_env["dirs"]

    # 1. Unauthenticated request must fail
    res = client.get("/webmanager/status")
    assert res.status_code == 401

    # 2. Authenticated status check
    auth_headers = {"Authorization": f"Bearer {token}"}
    res = client.get("/webmanager/status", headers=auth_headers)
    assert res.status_code == 200
    status_data = res.get_json()
    assert status_data["status"] == "ok"
    assert status_data["readonly"] is False

    # 3. Path containment verification on /webmanager/import
    outside_path = os.path.join(dirs["root"], "..", "etc", "passwd")
    res = client.post(
        "/webmanager/import",
        headers=auth_headers,
        json={"paths": [outside_path]},
    )
    assert res.status_code == 400
    assert "Path traversal or disallowed root" in res.get_json()["error"]

    # 4. Modify tags
    res = client.post(
        "/webmanager/modify",
        headers=auth_headers,
        json={
            "item_ids": [item.id],
            "fields": {"title": "Sands of Time (Remastered)", "genre": "Chillout"},
            "write": False,
            "move": False,
        },
    )
    assert res.status_code == 200
    assert res.get_json()["success"] is True

    # 5. Verify modification through native upstream read
    res = client.get(f"/item/{item.id}")
    assert res.status_code == 200
    assert res.get_json()["title"] == "Sands of Time (Remastered)"
    assert res.get_json()["genre"] == "Chillout"


@pytest.mark.skipif(not shutil.which("docker"), reason="Docker CLI not available on host")
def test_real_stock_docker_container_acceptance():
    """Real Docker Acceptance Test booting unmodified lscr.io/linuxserver/beets:latest.

    Tests:
    1. Stock image startup with web & webmanager plugins mounted in /config
    2. Native Beets GET /stats and GET /item/
    3. WebManager plugin authentication (rejecting invalid token, accepting valid token)
    4. Non-interactive import of a synthetic tagged MP3 file from /downloads into /music
    5. Idempotency test (lost response replay returning same result without duplicate import)
    6. Operation ID collision safety (rejecting conflicting payload with 409 Conflict)
    7. Upstream native verification of the imported file and fields
    8. Disposable mutation modification and upstream verification
    """
    container_name = f"stock-beets-acc-{uuid.uuid4().hex[:8]}"
    repo_root = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))

    with tempfile.TemporaryDirectory() as td:
        config_dir = os.path.join(td, "config")
        music_dir = os.path.join(td, "music")
        downloads_dir = os.path.join(td, "downloads")
        os.makedirs(config_dir, exist_ok=True)
        os.makedirs(music_dir, exist_ok=True)
        os.makedirs(downloads_dir, exist_ok=True)

        # 1. Provision webmanager plugin into config_dir/beetsplug/webmanager
        target_plugin_dir = os.path.join(config_dir, "beetsplug", "webmanager")
        os.makedirs(target_plugin_dir, exist_ok=True)
        src_plugin_dir = os.path.join(repo_root, "beetsplug", "webmanager")
        for f in ["__init__.py", "compat.py", "auth.py", "schemas.py", "operations.py"]:
            shutil.copy2(os.path.join(src_plugin_dir, f), os.path.join(target_plugin_dir, f))

        # 2. Provision secret API key file
        token = "real_docker_acceptance_secret_token_abcdef123456"
        key_file = os.path.join(config_dir, ".webmanager_api_key")
        with open(key_file, "w", encoding="utf-8") as f:
            f.write(token + "\n")

        # 3. Write config.yaml
        config_yaml = f"""plugins: web webmanager
pluginpath:
  - /config/beetsplug
directory: /music
library: /config/musiclibrary.blb
import:
  write: yes
  copy: no
  move: yes
  quiet: yes
  autotag: no
web:
  host: 0.0.0.0
  port: 8337
  readonly: yes
  include_paths: yes
webmanager:
  api_key_file: /config/.webmanager_api_key
  allowed_roots:
    - /music
    - /downloads
"""
        with open(os.path.join(config_dir, "config.yaml"), "w", encoding="utf-8") as f:
            f.write(config_yaml)

        # 4. Generate synthetic tagged audio file in downloads
        track_name = "Synthetic_Acceptance_Track.wav"
        track_path = os.path.join(downloads_dir, track_name)
        _create_synthetic_audio(
            track_path,
            title="Synthetic Anthem",
            artist="Acceptance Bot",
            album="Docker Test LP",
        )

        # 5. Start stock Beets container
        # Choose a random high port
        import socket
        sock = socket.socket()
        sock.bind(("", 0))
        host_port = sock.getsockname()[1]
        sock.close()

        run_cmd = [
            "docker", "run", "-d",
            "--name", container_name,
            "-p", f"{host_port}:8337",
            "-v", f"{config_dir}:/config",
            "-v", f"{music_dir}:/music",
            "-v", f"{downloads_dir}:/downloads",
            "-e", "PUID=1000",
            "-e", "PGID=1000",
            STOCK_BEETS_IMAGE,
        ]

        subprocess.check_call(run_cmd)

        try:
            base_url = f"http://127.0.0.1:{host_port}"
            # Wait for Beets web server to become responsive
            responsive = False
            for _ in range(30):
                time.sleep(1)
                try:
                    req = urllib.request.Request(f"{base_url}/stats")
                    with urllib.request.urlopen(req, timeout=2) as resp:
                        if resp.status == 200:
                            responsive = True
                            break
                except Exception:
                    pass

            assert responsive, f"Timed out waiting for stock Beets container at {base_url}"

            # Step 1: Upstream Native Read Check
            req = urllib.request.Request(f"{base_url}/stats")
            with urllib.request.urlopen(req, timeout=5) as resp:
                stats = json.loads(resp.read().decode("utf-8"))
                assert "items" in stats

            # Step 2: WebManager Auth Check
            # Invalid key must return 401
            try:
                req = urllib.request.Request(
                    f"{base_url}/webmanager/status",
                    headers={"Authorization": "Bearer wrong_token"},
                )
                urllib.request.urlopen(req, timeout=5)
                pytest.fail("Expected 401 for wrong token")
            except urllib.error.HTTPError as ex:
                assert ex.code == 401

            # Valid key must return 200 OK
            auth_header = {"Authorization": f"Bearer {token}", "Content-Type": "application/json"}
            req = urllib.request.Request(f"{base_url}/webmanager/status", headers=auth_header)
            with urllib.request.urlopen(req, timeout=5) as resp:
                status_res = json.loads(resp.read().decode("utf-8"))
                assert status_res["status"] == "ok"
                assert "beets_version" in status_res

            # Step 3: Non-interactive Import of synthetic media with Idempotency Key
            idemp_key = "test-idemp-key-docker-001"
            import_payload = {
                "paths": [f"/downloads/{track_name}"],
                "autotag": False,
                "duplicate_action": "skip",
                "singletons": True,
                "copy": False,
                "move": True,
                "write": True,
            }
            import_headers = dict(auth_header)
            import_headers["Idempotency-Key"] = idemp_key

            req = urllib.request.Request(
                f"{base_url}/webmanager/import",
                data=json.dumps(import_payload).encode("utf-8"),
                headers=import_headers,
                method="POST",
            )
            with urllib.request.urlopen(req, timeout=15) as resp:
                assert resp.status in (200, 202)
                import_res = json.loads(resp.read().decode("utf-8"))
                assert import_res.get("success") is True or "operation_id" in import_res

            # Step 4: Idempotency Replay (Lost Response Scenario)
            # Re-issue exact same request with same Idempotency-Key
            req_replay = urllib.request.Request(
                f"{base_url}/webmanager/import",
                data=json.dumps(import_payload).encode("utf-8"),
                headers=import_headers,
                method="POST",
            )
            with urllib.request.urlopen(req_replay, timeout=15) as resp:
                assert resp.status in (200, 202)

            # Step 5: Collision Check (Same Idempotency-Key with DIFFERENT Payload)
            conflicting_payload = dict(import_payload)
            conflicting_payload["duplicate_action"] = "remove"
            req_conflict = urllib.request.Request(
                f"{base_url}/webmanager/import",
                data=json.dumps(conflicting_payload).encode("utf-8"),
                headers=import_headers,
                method="POST",
            )
            try:
                urllib.request.urlopen(req_conflict, timeout=15)
                pytest.fail("Expected 409 Conflict for differing payload with same idempotency key")
            except urllib.error.HTTPError as ex:
                assert ex.code == 409

            # Step 6: Verify imported track in native upstream Beets Web API
            req_items = urllib.request.Request(f"{base_url}/item/")
            with urllib.request.urlopen(req_items, timeout=5) as resp:
                items_res = json.loads(resp.read().decode("utf-8"))
                items = items_res.get("items", [])
                assert len(items) >= 1
                imported_item = items[0]
                assert imported_item["title"] == "Synthetic Anthem"
                assert imported_item["artist"] == "Acceptance Bot"
                # Verify include_paths displays path in /music
                assert "/music" in imported_item.get("path", "")

            # Step 7: Exercise Disposable Mutation (POST /webmanager/modify)
            modify_payload = {
                "item_ids": [imported_item["id"]],
                "fields": {"genre": "Synthesized Electro"},
                "write": False,
                "move": False,
            }
            req_mod = urllib.request.Request(
                f"{base_url}/webmanager/modify",
                data=json.dumps(modify_payload).encode("utf-8"),
                headers=auth_header,
                method="POST",
            )
            with urllib.request.urlopen(req_mod, timeout=10) as resp:
                mod_res = json.loads(resp.read().decode("utf-8"))
                assert mod_res["success"] is True

            # Step 8: Verify mutation through native upstream GET /item/<id>
            req_verify = urllib.request.Request(f"{base_url}/item/{imported_item['id']}")
            with urllib.request.urlopen(req_verify, timeout=5) as resp:
                ver_res = json.loads(resp.read().decode("utf-8"))
                assert ver_res["genre"] == "Synthesized Electro"

        finally:
            # Clean up container
            subprocess.run(["docker", "rm", "-f", container_name], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
