...
class RootFolderRepairApplySafeTests(unittest.TestCase):
    ...
    def test_shallow_items_use_beet_write_then_move_per_item(self):
        self.assertIn('composite_workflows.update_album_metadata(', self._apply)
        self.assertIn('composite_workflows.relocate_album(', self._apply)

    def test_mbsync_only_when_item_has_mb_ids(self):
        self.assertIn("has_mb", self._apply)
        self.assertIn('composite_workflows.plan_album_mb_track_repair(', self._apply)
        self.assertIn('composite_workflows.apply_album_mb_track_repair(', self._apply)
