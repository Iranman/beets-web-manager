...
class CleanAllWorkflowStaticTests(unittest.TestCase):
    ...
    def test_clean_all_runs_identity_merge_and_duplicate_phases(self):
        ...
        # folder merge step now calls the shared step helper directly
        # (so it can persist/detect an engine operation_id across a
        # process restart) instead of invoking the stamp-mbid route via a
        # synthetic Flask request context.
        self.assertIn('_maintenance_artist_folder_merge_step(', APP_SOURCE)
        self.assertIn('resume_operation_id=resume_op_id', APP_SOURCE)
        self.assertIn('on_operation_planned=_persist_artist_folder_merge_operation_id', APP_SOURCE)
