...
const STEPS = [
  ...
  'Prepare MusicBrainz submission',
  'Confirm published MusicBrainz IDs',
  'Generate and submit AcoustID fingerprints',
  'Verify and complete',
];
...
function sourceChip(label: string) {
  return <Chip size="small" variant="outlined" label={label} sx={{ height: 22, fontSize: '0.68rem' }} />;
}
...
function stat(label: string, value?: string | number | null) {
  return (
    <div className="rounded border border-graphite-800 bg-graphite-950/45 px-3 py-2">
      <div className="text-[0.68rem] uppercase tracking-wide text-zinc-500">{label}</div>
      <div className="mt-1 truncate text-sm text-zinc-200">{value || 'Missing'}</div>
    </div>
  );
}
...
export default function Submissions() {
  ...
  useEffect(() => {
    let cancelled = false;
    if (!activeAlbumId && !activeItemId) {
      setTarget(null);
      return undefined;
    }
    setTargetLoading(true);
    getSubmissionTarget({ albumId: activeAlbumId || undefined, itemId: activeItemId || undefined })
      .then((data) => {
        if (cancelled) return;
        setTarget(data);
        const remoteDraft = (data.draft || {}) as Partial<DraftState>;
        setDraft({ metadata: remoteDraft.metadata || {}, trackEdits: remoteDraft.trackEdits || {}, stage: remoteDraft.stage, mbsubmit_output: remoteDraft.mbsubmit_output, published: remoteDraft.published });
  ...
  }, [activeAlbumId, activeItemId]);

  useEffect(() => {
    if (!dirty || !target) return undefined;
    if (saveTimer.current) window.clearTimeout(saveTimer.current);
    saveTimer.current = window.setTimeout(() => {
      void saveSubmissionDraft({ target_type: target.target_type, target_id: target.target_id, draft }).then(() => setDirty(false)).catch((err) => setMessage(err instanceof Error ? err.message : String(err)));
    }, 800);
  ...
  const primary = (() => {
    if (!target) return { label: 'Select an item', disabled: true, action: () => undefined };
        ...
        <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
          <div>
            ...
            <div className="mt-2 flex flex-wrap gap-2">
              ...
              <Chip label={`${tracks.length || selectedItem?.tracks || 0} tracks`} size="small" variant="outlined" />
            </div>
          </div>
              ...
              return (
                <button key={id} type="button" onClick={() => selectItem(item)} className={['w-full rounded border px-3 py-2 text-left transition-colors', selected ? 'border-red-500 bg-red-950/35 text-zinc-100' : 'border-graphite-800 bg-graphite-950/40 text-zinc-300 hover:border-graphite-700 hover:bg-graphite-850'].join(' ')}>
                  ...
                  <div className="mt-2 grid grid-cols-2 gap-1 text-[0.68rem] text-zinc-400">
                    <span>{item.tracks || item.evidence?.folder?.track_count || 0} tracks</span>
        ...
        <div className="space-y-4">
          <section className="rounded-md border border-graphite-800 bg-graphite-900/70 p-4">
            <div className="flex flex-col gap-3 xl:flex-row xl:items-start xl:justify-between">
              <div className="min-w-0">
                ...
                <div className="mt-2 flex flex-wrap gap-2">{sourceChip('Beets')}{draft.metadata && Object.keys(draft.metadata).length ? sourceChip('Draft edits') : null}{selectedCandidateCount ? sourceChip(`${selectedCandidateCount} MB candidates`) : null}</div>
              </div>
            ...
            <div className="mt-4 grid gap-2 md:grid-cols-3 xl:grid-cols-4">
              {stat('Release type', target?.summary.release_type)}{stat('Status', target?.summary.release_status)}{stat('Date', target?.summary.release_date)}{stat('Country', target?.summary.country)}
              {stat('Label', target?.summary.label)}{stat('Catalog', target?.summary.catalog_number)}{stat('Barcode', target?.summary.barcode)}{stat('Format', target?.summary.format)}
              {stat('Discs', target?.summary.disc_count)}{stat('Tracks', target?.summary.track_count)}{stat('Runtime', target?.summary.runtime_display)}{stat('Source', target?.summary.source_path)}
            </div>
          ...
          <section className="rounded-md border border-graphite-800 bg-graphite-900/70 p-4">
            <div className="grid gap-2 lg:grid-cols-6">
              {STEPS.map((label, index) => <div key={label} className={['rounded border px-3 py-2 text-xs', index === activeStep ? 'border-red-500 bg-red-950/35 text-zinc-100' : index < activeStep ? 'border-emerald-800 bg-emerald-950/20 text-emerald-200' : 'border-graphite-800 bg-graphite-950/35 text-zinc-500'].join(' ')}>{index + 1}. {label}</div>)}
            </div>
          </section>
          ...
          <section className="rounded-md border border-graphite-800 bg-graphite-900/70 p-4">
            <div className="flex items-center justify-between gap-3"><h2 className="text-sm font-semibold text-zinc-100">Editable Release Metadata</h2>{dirty ? <Chip size="small" label="Saving draft" color="warning" variant="outlined" /> : <Chip size="small" label="Draft saved" color="success" variant="outlined" />}</div>
            <div className="mt-3 grid gap-3 md:grid-cols-3">
              {[['title', 'Release title'], ['albumartist', 'Release artist credit'], ['release_type', 'Primary release type'], ['secondary_type', 'Secondary type'], ['release_status', 'Status'], ['release_date', 'Release date'], ['country', 'Country'], ['label', 'Label'], ['catalog_number', 'Catalog number'], ['barcode', 'Barcode'], ['format', 'Media format'], ['annotation', 'Annotation or note']].map(([key, label]) => <TextField key={key} label={label} size="small" value={draft.metadata[key] ?? String((target?.summary as Record<string, unknown> | undefined)?.[key] ?? '')} onChange={(event) => updateMeta(key, event.target.value)} helperText={draft.metadata[key] ? 'User-edited draft' : 'Read from Beets/file tags'} />)}
            </div>
          </section>

          <section className="rounded-md border border-graphite-800 bg-graphite-900/70 p-4">
            <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between"><h2 className="text-sm font-semibold text-zinc-100">Track Submission Preview</h2><div className="flex flex-wrap gap-2"><Button size="small" variant="outlined" onClick={() => void copyText(parserText, 'Track-parser text')}>Copy track-parser text</Button><Button size="small" variant="outlined" onClick={() => void copyText(`${target?.summary.albumartist || ''} - ${target?.summary.title || ''}\n\n${parserText}`, 'Submission summary')}>Copy complete summary</Button></div></div>
