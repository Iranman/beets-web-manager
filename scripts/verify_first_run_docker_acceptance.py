...
a fresh directory tree (state must not bleed between them):
  ...
  1. fresh_install   -- empty persistent dirs, no password configured.
     Covers acceptance items 1-14, 17-18 below; proves the generated
     initial password itself survives restart and force-recreate
     (byte-for-byte file comparison + re-authentication, not just file
     existence); then runtime password replacement (change with no
     restart, then restart, then force-recreate, proving the replacement
     survives both and the old generated password stays invalid).
...
def _write_secure_env_file(path: Path, env: dict) -> None:
    """Write a disposable, synthetic-credential-only .env for one throwaway
    container lifecycle, 0600 from the moment of creation (not chmod'd
    afterward -- no window where it's world/group readable). Every value
    here is a hardcoded fake token/password generated by this script for a
    single disposable test run (see the "-not-real-" / "test-firstrun" /
    "Simulated" naming throughout this file), never a real developer or
    host credential, and this content is never printed anywhere. The whole
    tempdir containing this file -- not just this file -- is deleted in
    Stack.down(), which every scenario calls from a `finally` block, so
    teardown happens even when a scenario fails partway through.
    CodeQL's py/clear-text-storage-sensitive-data still flags writing a
    password-shaped value to disk at all, regardless of permissions or
    lifetime -- suppressed on the write below because both are exactly
    synthetic disposable test data, not application secret handling.
    """
    content = "\n".join(f"{k}={v}" for k, v in env.items()) + "\n"
    fd = os.open(str(path), os.O_WRONLY | os.O_CREAT | os.O_TRUNC, 0o600)
    with os.fdopen(fd, "w", encoding="utf-8") as fh:
        fh.write(content)  # lgtm[py/clear-text-storage-sensitive-data] -- synthetic disposable test credentials only; 0600, deleted by Stack.down()'s finally-guaranteed teardown, never printed.
    try:
        os.chmod(path, 0o600)  # belt-and-suspenders; a no-op on platforms where os.open's mode already applied it
    except OSError:
        pass
...
def scenario_fresh_install():
    ...
    try:
        ...
        # ---- Initial generated password must itself survive restart and
        # force-recreate, BEFORE any replacement -- proven live against the
        # real container here, not inferred from a unit test. ----
        print("-- initial generated password: restart/force-recreate persistence (before replacement) --")
        initial_file_before = read_container_file(stack, "beets-web-manager", initial_path)
        if initial_file_before == initial_pwd:
            ok("captured initial password matches the persisted file's contents before restart")
        else:
            fail("initial password value and persisted file contents disagree before restart")

        run(["docker", "restart", "beets-web-manager"])
        if stack.wait_healthy("beets-web-manager"):
            ok("beets-web-manager healthy again after restart (initial password, pre-replacement)")
        else:
            fail("beets-web-manager did not become healthy after restart (initial password, pre-replacement)")
        after_restart = read_container_file(stack, "beets-web-manager", initial_path)
        if after_restart == initial_pwd:
            ok("initial password file byte-for-byte identical after restart")
        else:
            fail(f"initial password file changed after restart (before={initial_pwd!r} after={after_restart!r})")
        code, _, _ = stack.request("/api/setup/status", auth_header=basic_header("admin", initial_pwd))
        if code == 200:
            ok("initial password still authenticates (200) after restart")
        else:
            fail(f"initial password no longer authenticates after restart ({code})")

        stack.compose("up", "-d", "--force-recreate", "beets-web-manager")
        if stack.wait_healthy("beets-web-manager"):
            ok("beets-web-manager healthy again after force-recreate (initial password, pre-replacement)")
        else:
            fail("beets-web-manager did not become healthy after force-recreate (initial password, pre-replacement)")
        after_recreate = read_container_file(stack, "beets-web-manager", initial_path)
        if after_recreate == initial_pwd:
            ok("initial password file byte-for-byte identical after force-recreate")
        else:
            fail(f"initial password file changed after force-recreate (before={initial_pwd!r} after={after_recreate!r})")
        code, _, _ = stack.request("/api/setup/status", auth_header=basic_header("admin", initial_pwd))
        if code == 200:
            ok("initial password still authenticates (200) after force-recreate")
        else:
            fail(f"initial password no longer authenticates after force-recreate ({code})")
