...
7. (Optional) If your Beets engine container uses custom staging/intake mount paths (e.g. `/data/downloads` or `/srv/intake`), configure `BEETS_IMPORT_SOURCE_ROOTS` in `.env` so the Web UI dynamically exposes and validates those locations.
