...
* **Cause**: Docker encountered a local-only Beets engine image name (`beets-engine:local` or `beets-engine:dev`) without a local build context.
* **Fix**: Do **NOT** run `docker login` (this is not an authentication issue).
  1. For standard deployments: Use the production `docker-compose.yml` (which uses published GHCR images `ghcr.io/iranman/beets-engine:${BEETS_WEB_MANAGER_VERSION:-stable}`).
  2. For standalone web-manager deployments connecting to an external Beets instance: Use `examples/docker-compose.external-beets.yml`.
  3. For source builds: Run `docker compose -f docker-compose.dev.yml up -d --build` directly from the repository root.
