...
- `beets` (Beets Engine Container): authoritative Beets installation built from `Dockerfile.beets`, based on `lscr.io/linuxserver/beets:latest` (deliberately floating, not version/digest-pinned, since 2026-08-05 -- see `scripts/validate_compose_security.py`'s `APPROVED_BEETS_BASE_IMAGES` comment for the trade-off). Contains the Beets CLI, `/config/config.yaml`, `/config/musiclibrary.blb`, bundled plugins (`/opt/beets-web-manager-agent/beetsplug/discpath.py`), and the HTTP control agent (`backend/beets_control_agent.py`) supervised under S6 (`/custom-services.d/beets-control-agent`). Port 8338 is internal-only.
